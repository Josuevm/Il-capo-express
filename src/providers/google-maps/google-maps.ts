...
export class GoogleMapsProvider {
  ...
  getUserOrDefaultLocation(){
    ...
    this.geolocation.getCurrentPosition().then((position) => {
      firebase.auth().onAuthStateChanged(user => {
        if (user && self.loggedUser.address.position.lat) {
          self.getUserPosition(user.uid).then(user =>{
            
            self.latLng = new google.maps.LatLng(
              Number(self.loggedUser.address.position.lat), 
  ...
  getUserPosition(userUID): Promise<any>{
    return new Promise ((user) =>{let self = this;
    let doc = this.db.getDocument('users', userUID);
    doc.on("value", function (snapshot) {
      let data = snapshot.val()
      self.loggedUser.address.position.lat = data.address.position.lat;
      self.loggedUser.address.position.lng = data.address.position.lng;
    })
  });
  }
}
